...
def munculmonster_arena(monsterarray: list, level: int): # Untuk menampilkan data monster yang muncul
...
bot_battle_monster_level = 1 # Inisiasi level monster bot, sama kaya nomor stage

total_damage_dealt = 0 # Buat stats di akhir
total_damage_taken = 0 # Buat stats di akhir
total_OC = 0

while bot_battle_monster_level <= 5:
    # Loop untuk semua stage
    # Reset HP setiap kali ganti stage
    ...
    # Masukin stats monster
    monster_muncul = generate_number(lcg(a=48271, c=0, m=2**31-1, seed=None), [1,len(monsterarray)]) # Randomize monster yang muncul
    print(f"============= STAGE {bot_battle_monster_level} =============")
    munculmonster_arena(monsterarray, bot_battle_monster_level) # Jan lupa masukin ke base stats buat dihitung di battle nya, lanjutin nanti lagi
    bot_battle_monster = monsterarray[monster_muncul].copy() # Memasukkan data array monster bot yang akan melawan user dalam 1 stage
    bot_battle_monster = level_buff(bot_battle_monster_level, bot_battle_monster) # Mengubah atribut monster bot menjadi integer (level musuh nambah 1 setiap nambah stage)
    ...
    # Proses battle dalam satu stage
    ...
    while base_HP_user_monster > 0 and base_HP_bot_monster > 0:
        ...
        while(True):
            ...
            if user_action == 1: # Jika user memilih attack
                ...
                # Masukin damage_after_reduction ke total_damage_dealt
                total_damage_dealt += damage_after_reduction
        ...
        # Masukin damage_after_reduction punya bot ke total_damage_taken
        total_damage_taken += damage_after_reduction
    ...
    # Selesai satu stage
    # Kasus menang
    if base_HP_bot_monster == 0 and bot_battle_monster_level == 1: # Menang stage 1 
        OC = 30
        print(f"Selamat, Anda berhasil mengalahkan monster {bot_battle_monster[1]} !!!")
        print()
        print(f"STAGE CLEARED! Anda akan mendapatkan {OC} OC pada sesi ini!")
        print()
        print("Memulai stage berikutnya...")
        bot_battle_monster_level += 1
        total_OC += OC
    elif base_HP_bot_monster == 0 and bot_battle_monster_level == 2: # Menang stage 2
        OC = 50
        print(f"Selamat, Anda berhasil mengalahkan monster {bot_battle_monster[1]} !!!")
        print()
        print(f"STAGE CLEARED! Anda akan mendapatkan {OC} OC pada sesi ini!")
        ...
        bot_battle_monster_level += 1
        total_OC += OC
    elif base_HP_bot_monster == 0 and bot_battle_monster_level == 3: # Menang stage 3
        OC = 100
        print(f"Selamat, Anda berhasil mengalahkan monster {bot_battle_monster[1]} !!!")
        print()
        print(f"STAGE CLEARED! Anda akan mendapatkan {OC} OC pada sesi ini!")
        ...
        bot_battle_monster_level += 1
        total_OC += OC
    elif base_HP_bot_monster == 0 and bot_battle_monster_level == 4: # Menang stage 4
        OC = 200
        print(f"Selamat, Anda berhasil mengalahkan monster {bot_battle_monster[1]} !!!")
        print()
        print(f"STAGE CLEARED! Anda akan mendapatkan {OC} OC pada sesi ini!")
        ...
        bot_battle_monster_level += 1
        total_OC += OC
    elif base_HP_bot_monster == 0 and bot_battle_monster_level == 5: # Menang stage 5
        OC = 500
        total_OC += OC
        print(f"Selamat, Anda berhasil mengalahkan monster {bot_battle_monster[1]} !!!")
        print()
...
""")
    ...
    # Kasus kalah atau quit
        ...
        print()
        print(f"GAME OVER! Sesi latihan berakhir pada stage {bot_battle_monster_level}!")
        print()
        print(f"""
============== STATS ==============
Total hadiah      : {total_OC} OC
Jumlah stage      : {bot_battle_monster_level}
Damage diberikan  : {total_damage_dealt}
Damage diterima   : {total_damage_taken}
""")
        break
